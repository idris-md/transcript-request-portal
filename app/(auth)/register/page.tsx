...
export default function RegisterPage() {
  ...
  const fullName =
    profile &&
    `${profile.surname} ${profile.first_name ?? ""} ${profile.other_name ?? ""}`
      .replace(/\s+/g, " ")
      .trim();
  ...
  async function onLookupSubmit(values: LookupFormValues) {
    ...
    try {
      ...
      setStep(2);
      // Reset step-2 form in case user comes back
      resetRegisterForm();
    } catch (e) {
  ...
  async function onRegisterSubmit(values: RegisterFormValues) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setServerError(j.message || "An account with this matric already exists.");
        } else {
          setServerError(j.message || "Could not create account. Please try again.");
        }
        return;
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold">Transcript Portal Registration</h1>
        <p className="text-sm opacity-75">
          Verify your details from the e-Portal, then create a password for the transcript system.
        </p>
      </div>
      ...
      {step === 1 && (
        <Card className="border border-default-200/60 shadow-sm">
          <CardHeader className="flex flex-col items-start gap-1">
            <p className="text-base font-medium">Step 1 · Verify your matric number</p>
            <p className="text-xs opacity-70">
              We will pull your official student record (name, department, school, level, entry session).
            </p>
          </CardHeader>
          <CardBody>
            <form
        ...
        <Card className="border border-default-200/60 shadow-sm">
          <CardHeader className="flex flex-col gap-2">
            <div className="flex items-center justify-between gap-2">
              <div>
                <p className="text-base font-medium">Step 2 · Confirm details & set password</p>
                <p className="text-xs opacity-70">
                  Your profile is read from the e-Portal. If anything is wrong, contact the ICT/Records unit.
                </p>
              </div>
              ...
              >
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <Button
                    ...
                    size="sm"
                    onPress={() => {
                      // allow going back to correct matric
                      setStep(1);
                      setProfile(null);
                      setServerError(null);
                      setServerMessage(null);
